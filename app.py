...
2. អាន Knowledge Base ពី knowledge/*.txt ដាក់ចូល Prompt ដោយផ្ទាល់ (គ្មាន retrieval package ដែលធ្វើឲ្យ Build យឺត)
...
def load_knowledge_base(folder="knowledge"):
    """អាន Q&A ដែលបំពេញរួច ហើយបញ្ចូលគ្នាជាអត្ថបទតែមួយ (មិនរាប់ [សូមបំពេញ...])"""
    qa_text_blocks = []
    for filepath in glob.glob(os.path.join(folder, "*.txt")):
        ...
        for question, answer in blocks:
            ...
            if answer.startswith("[") and answer.endswith("]"):
                continue
            qa_text_blocks.append(f"សំណួរ: {question.strip()}\nចម្លើយ: {answer}")
    return "\n\n".join(qa_text_blocks)
